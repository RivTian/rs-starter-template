...
```mermaid
graph TD
    BIN["acme-svc (bin)<br/>CLI · config · runtime · bootstrap"]
    API["acme-svc-api<br/>HTTP transport"]
    INFRA["acme-svc-infra<br/>adapters · jobs"]
    CORE["acme-svc-core<br/>Server · Service · shutdown · readiness · health · telemetry · EventBus"]
    DOMAIN["acme-svc-domain<br/>entities · ports · DomainError · DomainEvent"]
    CONFIG["acme-svc-config<br/>schema · load · validate"]
    UTIL["acme-svc-util<br/>Classify · ids · Secret"]
    TU["acme-svc-test-utils<br/>(dev only)"]
...
| 7 | `acme-svc/src/bootstrap.rs` | wire adapter → service → `AppState` |
| 8 | `acme-svc/tests/http_smoke.rs` | end-to-end assertion |
...
| Trigger | New crate |
|---|---|
| a second, independent bounded context | `acme-svc-domain-<name>` |
| an adapter with a heavy driver (database, message broker) | `acme-svc-infra-<driver>` |
| a second transport (gRPC, CLI) | `acme-svc-<transport>` next to `api` |
| a proc-macro | `acme-svc-macros` |
