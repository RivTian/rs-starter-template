...
use acme_svc_core::server::{PhaseWatch, Server};
use acme_svc_core::service::Service;
use acme_svc_core::telemetry::RuntimeMetricsService;
use acme_svc_core::telemetry::{self, PrometheusHandle};
...
pub fn wire(cfg: &Config, phase: PhaseWatch, metrics: Option<PrometheusHandle>) -> Wiring {
    ...
    let router = acme_svc_api::router::router(state.clone(), &cfg.http);
