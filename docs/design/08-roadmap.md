...
| D12 `license` 选项含 `MIT OR Apache-2.0`                         | 改为 `MIT` / `Apache-2.0` / `None`（不生成 LICENSE、不写 `license` 字段；`deny.toml` 用 `[licenses.private] ignore = true` 放过私有 crate） | 用户试用后的决定（2026-09-20）：内部项目常常不需要许可证，双许可反而少见                    |
