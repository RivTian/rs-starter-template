//! Configuration schema and layered loading for `{{project-name}}`.
//!
...
//! 3. environment variables `{{env_prefix}}__<SECTION>__<KEY>` (double underscore separates levels),
...
pub struct Sources {
    ...
    /// Environment prefix, e.g. `{{env_prefix}}` for `{{env_prefix}}__HTTP__BIND`.
