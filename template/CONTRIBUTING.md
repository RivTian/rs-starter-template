...
|---|---|---|
| a type with rules (invariants) | `{{project-name}}-domain` | rules must not depend on I/O |
| something the domain needs from outside (storage, time, messaging) | a trait in `{{project-name}}-domain::ports` | the domain owns its ports |
| an implementation of such a trait | `{{project-name}}-infra` | adapters may depend on drivers |
| an HTTP route, DTO or middleware | `{{project-name}}-api` | transport only |
| a background job | `{{project-name}}-infra::jobs`, implementing `core::Service` | jobs are services |
| a configuration key | `{{project-name}}-config::sections` + `config/default.toml` | one source of truth |
| a lifecycle / observability facility | `{{project-name}}-core` | must stay business-free |
| a pure helper needed by ≥ 2 crates | `{{project-name}}-util` | otherwise keep it local |
...
* Log errors with their cause chain: `error = %chain(&e)` (`{{crate_name}}_core::error::chain`). A bare
